...
class AttnGANDataset(Dataset):
    def __init__(self, data_dir: str, split: str = "train", transform: Optional[Compose] = None):
        ...
        self.class_ids_path = os.path.join(self.split_dir, "class_info.pickle")
        ...
        self.class_ids = self._load_class_id()
    ...
    def _load_class_id(self):
        if os.path.isfile(self.class_ids_path):
            with open(self.class_ids_path, 'rb') as file:
                return pickle.load(file, encoding="bytes")
        raise ValueError("No class ids")
    ...
    def _get_image(self, image_path: str, bbox: List[int]) -> List[Tensor]:
        ...
        for size in self.image_sizes:
            images.append(self.normalize(transforms.Scale(size)(image)))
    ...
    def __getitem__(self, idx: int):
        ...
        return image, encoded_caption, caption_len, self.class_ids[idx], file_name
